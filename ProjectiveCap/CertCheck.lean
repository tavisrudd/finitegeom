...
Generated files provide concrete list-backed books.  This reflection layer evaluates them with
...
/-- Short-circuiting universal Boolean quantification over a list. -/
def allShort {α : Type*} (p : α -> Bool) : List α -> Bool
  | [] => true
  | x :: xs => if p x then allShort p xs else false

/-- Short-circuiting existential Boolean quantification over a list. -/
...
omit [Fintype K] in
/-- A successful executable cap check proves the semantic residual-grid cap predicate. -/
...
omit [Fintype K] in
/-- A successful executable move check proves semantic legality in the residual-grid game. -/
...
/-- Interpret concrete list-backed book data as a semantic reply-book DAG. -/
...
/-- Check that the declared root occurs in the concrete node set. -/
def checkRoot (b : BookData K) : Bool :=
  decide (b.root.toFinset ∈ b.nodesFinset)

/-- Check that every concrete book node is a residual-grid cap. -/
def checkNodes (b : BookData K) : Bool :=
  allShort (fun S => checkCap (K := K) S) b.nodes

/-- Chunked form of `checkNodes`, used by generated certificate shards. -/
...
/-- Check membership in a concrete node list split into generated chunks. -/
...
/-- Check the responder condition at one node, with candidate cells split into chunks. -/
...
/-- Check one node using separately chunked node, row, and candidate-cell data. -/
...
/-- Check the complete responder condition at one concrete book node. -/
def checkNodeStep (b : BookData K) (S : List (GridPoint K)) : Bool :=
  checkNodeStepOn (K := K) b S b.cells

omit [Fintype K] in
/-- Checking every candidate-cell chunk implies the unchunked node-step check when the chunks
flatten to the book's complete cell list. -/
...
omit [Fintype K] in
/-- A successful chunked node check implies the unchunked node check when the chunks flatten to
the book's node list. -/
...
omit [Field K] [Fintype K] in
/-- A successful chunked membership check proves membership in the semantic node finset. -/
...
omit [Fintype K] in
/-- The fully chunked node-step check implies the ordinary node-step check when all chunk lists
flatten to the corresponding book data. -/
...
/-- Check the responder condition at every concrete book node. -/
def checkSteps (b : BookData K) : Bool :=
  allShort (fun S => checkNodeStep (K := K) b S) b.nodes

/-- Chunked form of `checkSteps`, used by generated certificate shards. -/
...
omit [Fintype K] in
/-- A successful chunked step check implies the unchunked step check when the chunks flatten to
the book's node list. -/
...
omit [Field K] [Fintype K] in
/-- A successful root check proves that the root belongs to the semantic node finset. -/
...
omit [Fintype K] in
/-- Successful cap checks for all concrete nodes prove the semantic node-cap condition. -/
...
/-- Successful responder checks for all nodes prove the semantic reply condition. -/
...
/-- Interpret concrete class data as a semantic residual-grid class certificate. -/
