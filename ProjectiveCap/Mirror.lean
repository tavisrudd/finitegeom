...
/-- Projective collinearity is invariant under exchanging its first two points. -/
...
omit [DecidableEq (Point K V)] in
/-- Projective collinearity is invariant under cyclic rotation. -/
...
omit [DecidableEq (Point K V)] in
/-- Projective collinearity is invariant under exchanging its final two points. -/
...
/-- A fresh projective point may be adjoined to a cap when it forms no
collinear triple with two distinct old points. -/
