...
/-!
# Characteristic-two mirror for the residual grid game

After the normalized projective frame, simultaneous translation of both grid
coordinates defines a fixed-point-free involution in characteristic two. The
module proves that this involution preserves grid caps and supplies a legal
mirror reply, yielding the residual P-position theorem.
-/
...
private theorem diagTranslate_involutive (h2 : (2 : K) = 0) (p : GridPoint K) :
    diagTranslate (K := K) (diagTranslate (K := K) p) = p := by
  ext <;> dsimp [diagTranslate] <;> linear_combination h2

private theorem diagTranslate_fpf (p : GridPoint K) :
...
private theorem diagTranslate_row_ne (p : GridPoint K) :
...
private theorem diagTranslate_col_ne (p : GridPoint K) :
...
private theorem diagEquiv_apply (h2 : (2 : K) = 0) (p : GridPoint K) :
    diagEquiv (K := K) h2 p = diagTranslate (K := K) p :=
  rfl

private theorem collinear_diagTranslate_iff (_h2 : (2 : K) = 0) (p q r : GridPoint K) :
...
/-- Grid collinearity is invariant under exchanging its first two points. -/
theorem collinear_swap_left {a b c : GridPoint K} :
    Collinear (K := K) a b c ↔ Collinear (K := K) b a c := by
  unfold Collinear
  constructor <;> intro h <;> linear_combination -h

/-- Grid collinearity is invariant under cyclic rotation. -/
theorem collinear_rotate {a b c : GridPoint K} :
    Collinear (K := K) a b c ↔ Collinear (K := K) b c a := by
  unfold Collinear
  constructor <;> intro h <;> linear_combination h

private theorem collinear_diagTranslate_left_iff (h2 : (2 : K) = 0) (p q r : GridPoint K) :
...
private theorem collinear_with_mirror_forces_old_line (h2 : (2 : K) = 0)
    {x p : GridPoint K}
...
private theorem mem_of_diag_image_eq_self (h2 : (2 : K) = 0) {S : Finset (GridPoint K)}
...
omit [Fintype K] [DecidableEq K] in
private theorem mem_iff_of_diag_image_eq_self (h2 : (2 : K) = 0)
...
omit [Fintype K] in
private theorem standardResidualSeed_diagMirrorGood (h2 : (2 : K) = 0) :
...
omit [Fintype K] in
private theorem old_row_ne_diag_reply (h2 : (2 : K) = 0)
...
omit [Fintype K] in
private theorem old_col_ne_diag_reply (h2 : (2 : K) = 0)
    {S : Finset (GridPoint K)} (hinv : S.map (diagEquiv (K := K) h2).toEmbedding = S)
    {x z : GridPoint K} (hxmove : GridGame.Move (K := K) S x) (hzS : z ∈ S) :
...
omit [Fintype K] in
private theorem rowSparse_insert_diag_reply (h2 : (2 : K) = 0)
    {S : Finset (GridPoint K)} (hinv : S.map (diagEquiv (K := K) h2).toEmbedding = S)
    {x : GridPoint K} (hxmove : GridGame.Move (K := K) S x) :
...
omit [Fintype K] in
private theorem colSparse_insert_diag_reply (h2 : (2 : K) = 0)
    {S : Finset (GridPoint K)} (hinv : S.map (diagEquiv (K := K) h2).toEmbedding = S)
    {x : GridPoint K} (hxmove : GridGame.Move (K := K) S x) :
...
omit [Fintype K] [DecidableEq K] in
private theorem diagTranslate_ne_of_ne (h2 : (2 : K) = 0) {p q : GridPoint K} (hpq : p ≠ q) :
...
omit [Fintype K] in
private theorem not_collinear_diag_reply_old_old (h2 : (2 : K) = 0)
    {S : Finset (GridPoint K)} (hinv : S.map (diagEquiv (K := K) h2).toEmbedding = S)
...
omit [Fintype K] in
private theorem affineCap_insert_diag_reply (h2 : (2 : K) = 0)
    {S : Finset (GridPoint K)} (hinv : S.map (diagEquiv (K := K) h2).toEmbedding = S)
    {x : GridPoint K} (hxmove : GridGame.Move (K := K) S x) :
...
omit [Fintype K] in
private theorem diag_mirror_move_legal (h2 : (2 : K) = 0) {S : Finset (GridPoint K)}
...
omit [Fintype K] in
private theorem diagMirrorGood_step (h2 : (2 : K) = 0) :
...
private theorem isP_of_diagMirrorGood (h2 : (2 : K) = 0) {S : Finset (GridPoint K)}
...
/-- In characteristic two, the normalized frame residual is a P-position. -/
...
/-- In the standard coordinate projective plane of characteristic two, the
normalized two-direction frame with residual seed `{(0,0), (1,1)}` is a
P-position for the fixed-prefix residual game. -/
