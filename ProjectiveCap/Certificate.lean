...
/-!
# Projective residual-grid certificate interface

This file connects externally serialized reply-book data to the formal residual
grid game.  Parsing remains outside the trusted kernel; generated Lean data must
construct the objects below and prove their stated validity obligations.
-/
...
The serialized file stores more audit metadata (`escape`, `onconic`, node/row
...
The witness must be a legal child of the size-three position, the book root
must be exactly that child, and the book must be a valid responder DAG for the
residual grid game.
-/
...
/-- A valid class certificate gives the escape witness required at its size-three
representative. -/
...
The serialized files are per canonical class.  This structure expresses their
assembly as a selector: for every legal size-three position, choose a class
certificate representing it.  The `represents` field carries the required
projective or canonical-orbit identification.
-/
...
/-- A full assembled book certificate proves the residual odd-escape property. -/
...
Generated books may certify anchored representatives.  This structure is the
transport-aware analogue of `GridOddEscapeBookCertificate`: for every legal
...
/--
A transported book certificate proves the residual odd-escape property.
-/
...
/-- Prime-field spelling of the assembled certificate property. -/
abbrev PrimeGridOddEscapeBookCertificate (p : ℕ) [Fact p.Prime] :=
  GridOddEscapeBookCertificate (ZMod p)

/--
For prime fields, a parsed and validated per-class book certificate proves the
`Almost` odd-escape property for `ZMod p`.

The serialized certificate format also supports `GF(9)`.  This theorem is
restricted to prime fields and therefore assumes neither a parsed extension-field
model nor an equivalence with an external polynomial representation.
-/
...
/--
For q=5, an assembled certificate proves the escape property.  The serialized
q=5 book has one class and a terminal root node.
-/
