...
The terminal theorem states the count directly in terms of
...
omit [Fintype K] [DecidableEq K] in
/-- Grid collinearity is invariant under exchanging its final two points. -/
...
private theorem legalExtensions_eq_filter_freeFree {S : Finset (GridPoint K)}
    (hS : GridCap (K := K) S) :
    GridGame.LegalExtensions (K := K) S =
      (FreeFreeCells (K := K) S).filter
        fun p => ¬ PairLineBlockedBy (K := K) S p := by
  classical
  ext p
  rw [GridGame.mem_legalExtensions, Finset.mem_filter, mem_freeFreeCells]
  constructor
  · rintro ⟨hpS, hcap⟩
    refine ⟨⟨?_, ?_⟩, ?_⟩
    · intro hrow
...
omit [Field K] in
private theorem freeFreeCells_eq_product (S : Finset (GridPoint K)) :
    FreeFreeCells (K := K) S =
...
omit [Field K] in
private theorem card_freeFreeCells (S : Finset (GridPoint K)) :
...
private theorem usedRows_triple (a b c : GridPoint K) :
    UsedRows (K := K) ({a, b, c} : Finset (GridPoint K)) = {a.1, b.1, c.1} := by
  simp [UsedRows, Finset.image_insert]

omit [Field K] [Fintype K] in
private theorem usedCols_triple (a b c : GridPoint K) :
...
omit [Fintype K] [DecidableEq K] in
private theorem collinear_iff_lineRowPoint {a b : GridPoint K} (hr : a.1 ≠ b.1)
    {p : GridPoint K} :
...
omit [Fintype K] [DecidableEq K] in
private theorem lineRowPoint_fst (a b : GridPoint K) : lineRowPoint a b a.1 = a.2 := by
  simp [lineRowPoint]

omit [Fintype K] [DecidableEq K] in
private theorem lineRowPoint_snd {a b : GridPoint K} (hr : a.1 ≠ b.1) :
...
omit [Fintype K] [DecidableEq K] in
private theorem lineRowPoint_injective {a b : GridPoint K} (hr : a.1 ≠ b.1)
...
private theorem card_pairLine_inter_freeFree {a b c : GridPoint K}
...
private theorem filter_blocked_triple {a b c : GridPoint K}
...
omit [Field K] [Fintype K] in
private theorem ne_of_fst_notMem_usedRows {S : Finset (GridPoint K)} {p t : GridPoint K}
...
private theorem card_legalExtensions_of_card_three {S : Finset (GridPoint K)}
  ...
  have hsplit :
      ((FreeFreeCells (K := K) ({a, b, c} : Finset (GridPoint K))).filter
            fun p => PairLineBlockedBy (K := K) ({a, b, c} : Finset (GridPoint K)) p).card +
          (GridGame.LegalExtensions (K := K) ({a, b, c} : Finset (GridPoint K))).card =
        (FreeFreeCells (K := K) ({a, b, c} : Finset (GridPoint K))).card := by
    rw [legalExtensions_eq_filter_freeFree hS]
  ...
  have zsplit : (((FreeFreeCells (K := K) ({a, b, c} : Finset (GridPoint K))).filter
        fun p => PairLineBlockedBy (K := K) ({a, b, c} : Finset (GridPoint K)) p).card : Int) +
      ((GridGame.LegalExtensions (K := K) ({a, b, c} : Finset (GridPoint K))).card : Int) =
      ((FreeFreeCells (K := K) ({a, b, c} : Finset (GridPoint K))).card : Int) := by
    exact_mod_cast hsplit
  rw [zbl, zFF] at zsplit
  linear_combination zsplit

/-- Every three-point residual grid cap has exactly
`q² - 9q + 21` legal one-point extensions, where `q` is the cardinality of
the coordinate field. -/
theorem sizeThreeExtensionCount (S : Finset (GridPoint K))
    (hcard : S.card = 3) (hS : GridCap (K := K) S) :
    ((GridGame.LegalExtensions (K := K) S).card : Int) =
      (Fintype.card K : Int) ^ 2 - 9 * (Fintype.card K : Int) + 21 :=
  card_legalExtensions_of_card_three hcard hS
