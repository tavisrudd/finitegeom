...
scalar is `1`. This identifies the projective cap game on `PG(n,2)` with the
sum-free placement game on nonzero vectors. The resulting transport proves
that every binary projective space of positive projective dimension is a
P-position.
-/
...
@[simp] private theorem binary_mk_rep (v : V) (hv : v ≠ 0) :
...
@[simp] private theorem mem_binaryRepFinset {S : Finset (Point (ZMod 2) V)} {x : V} :
...
omit [Fintype V] [DecidableEq V] [Fintype (Point (ZMod 2) V)]
  [DecidableEq (Point (ZMod 2) V)] in
@[simp] private theorem rep_mem_binaryRepFinset {S : Finset (Point (ZMod 2) V)}
    {p : Point (ZMod 2) V} :
