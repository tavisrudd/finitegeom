...
/-!
# Conic localization for residual grid caps

For a three-point residual grid cap, this module describes the conic through
the selected points and the two distinguished directions. It defines the
on-conic legal extensions, their game values, and the Möbius involutions used
to pair them. The terminal implication shows that a P-valued on-conic
extension supplies the ordinary residual-grid escape property.
-/
...
/-- Membership in `HyperbolaCells` is exactly satisfaction of the defining
hyperbola equation. -/
...
private theorem mem_nonzeroParams {t : K} :
    t ∈ NonzeroParams (K := K) ↔ t ≠ 0 := by
  simp [NonzeroParams]

private theorem card_nonzeroParams :
...
private theorem hyperbolaDenom_ne_zero_of_not_collinear {a b c : GridPoint K}
...
omit [Fintype K] in
private theorem hyperbolaDenom_ne_zero_of_gridCap_triple {a b c : GridPoint K}
...
omit [Fintype K] [DecidableEq K] in
private theorem hyperbolaTriple_linear_b {a b c : GridPoint K}
...
omit [Fintype K] [DecidableEq K] in
private theorem hyperbolaTriple_linear_c {a b c : GridPoint K}
    (hden : hyperbolaDenom (K := K) a b c ≠ 0) :
...
omit [Fintype K] [DecidableEq K] in
private theorem onHyperbola_pair_linear {rho A B : K} {p q : GridPoint K}
...
omit [Fintype K] [DecidableEq K] in
private theorem hyperbolaCenter_eq_of_linear {a b c : GridPoint K} {rho A : K}
    (hden : hyperbolaDenom (K := K) a b c ≠ 0)
...
omit [Fintype K] in
private theorem hyperbolaParams_eq_of_fits_triple {a b c : GridPoint K} {rho A B : K}
    (hden : hyperbolaDenom (K := K) a b c ≠ 0)
...
omit [Fintype K] [DecidableEq K] in
private theorem onHyperbola_hyperbolaOfTriple_a (a b c : GridPoint K) :
    OnHyperbola (K := K)
...
omit [Fintype K] [DecidableEq K] in
private theorem onHyperbola_hyperbolaOfTriple_b {a b c : GridPoint K}
    (hden : hyperbolaDenom (K := K) a b c ≠ 0) :
    OnHyperbola (K := K)
      (hyperbolaRhoOfTriple (K := K) a b c)
      (hyperbolaAOfTriple (K := K) a b c)
...
omit [Fintype K] [DecidableEq K] in
private theorem onHyperbola_hyperbolaOfTriple_c {a b c : GridPoint K}
    (hden : hyperbolaDenom (K := K) a b c ≠ 0) :
    OnHyperbola (K := K)
      (hyperbolaRhoOfTriple (K := K) a b c)
      (hyperbolaAOfTriple (K := K) a b c)
...
omit [Fintype K] in
private theorem hyperbolaFits_triple {a b c : GridPoint K}
    (hden : hyperbolaDenom (K := K) a b c ≠ 0) :
...
omit [Fintype K] [DecidableEq K] in
private theorem onAffine_iff_onHyperbola (p : GridPoint K) :
...
omit [Fintype K] in
private theorem hyperbolaFits_card_three_B_ne_zero {S : Finset (GridPoint K)} {rho A B : K}
...
omit [Fintype K] in
/-- Every three-point grid cap lies on a nondegenerate hyperbola in the
normalized coordinate family. -/
...
omit [Fintype K] in
private theorem hyperbolaNormalFormStatement :
...
omit [Fintype K] [DecidableEq K] in
private theorem hyperbolaConic_nondegenerate {rho A B : K} (hB : B ≠ 0) :
...
omit [Fintype K] [DecidableEq K] in
private theorem burnedDirectionConic_eq_hyperbolaConic_of_params
...
omit [Fintype K] [DecidableEq K] in
private theorem uniqueConicThroughFiveArcStatement_of_hyperbolaNormalFormStatement
...
omit [Fintype K] in
private theorem uniqueConicThroughFiveArcStatement :
...
/-- An on-conic legal extension is precisely a legal grid extension satisfying
the fitted hyperbola equation. -/
...
private theorem mem_onConicEscapeExtensions
    {S : Finset (GridPoint K)} {rho A B : K} {p : GridPoint K} :
...
private theorem mem_onConicBadExtensions
    {S : Finset (GridPoint K)} {rho A B : K} {p : GridPoint K} :
...
private theorem onConicLegalExtensions_card_eq_escape_add_bad
...
omit [Fintype K] [DecidableEq K] in
/-- The nonzero hyperbola parameter determines its grid point uniquely. -/
...
omit [Fintype K] [DecidableEq K] in
private theorem hyperbolaParamPoint_onHyperbola {rho A B t : K} (ht : t ≠ 0) :
...
omit [Fintype K] [DecidableEq K] in
/-- On a nondegenerate fitted hyperbola, the first coordinate never equals
the excluded center coordinate `rho`. -/
...
omit [Fintype K] [DecidableEq K] in
private theorem onHyperbola_second_ne_A {rho A B : K} (hB : B ≠ 0)
    {p : GridPoint K} (hp : OnHyperbola (K := K) rho A B p) :
    p.2 ≠ A := by
  intro h
  unfold OnHyperbola at hp
...
omit [Fintype K] [DecidableEq K] in
/-- Every point on a nondegenerate fitted hyperbola is recovered from the
difference between its first coordinate and `rho`. -/
...
omit [Fintype K] [DecidableEq K] in
private theorem onHyperbola_iff_exists_param {rho A B : K} (hB : B ≠ 0)
    {p : GridPoint K} :
    OnHyperbola (K := K) rho A B p ↔
...
private theorem hyperbolaCells_eq_image_nonzeroParams {rho A B : K} (hB : B ≠ 0) :
...
/-- A nondegenerate fitted hyperbola over `K` has `|K| - 1` grid points. -/
...
omit [DecidableEq K] in
/-- A nondegenerate fitted hyperbola contains at most one point in each row. -/
...
omit [DecidableEq K] in
private theorem colSparse_hyperbolaCells {rho A B : K} (hB : B ≠ 0) :
...
omit [DecidableEq K] in
private theorem partialPermutation_hyperbolaCells {rho A B : K} (hB : B ≠ 0) :
    PartialPermutation (K := K) (HyperbolaCells (K := K) rho A B) :=
  ⟨rowSparse_hyperbolaCells (K := K) hB, colSparse_hyperbolaCells (K := K) hB⟩

omit [Fintype K] [DecidableEq K] in
private theorem not_collinear_hyperbolaParamPoint {rho A B t s v : K}
...
omit [DecidableEq K] in
private theorem affineCap_hyperbolaCells {rho A B : K} (hB : B ≠ 0) :
...
omit [DecidableEq K] in
/-- The full point set of a nondegenerate fitted hyperbola is a residual grid
cap. -/
...
private theorem gridCap_hyperbolaCells_and_card {rho A B : K} (hB : B ≠ 0) :
    GridCap (K := K) (HyperbolaCells (K := K) rho A B) ∧
      ((HyperbolaCells (K := K) rho A B).card : Int) =
...
private theorem onConicLegalExtensionCountStatement :
...
private theorem onConicLegalExtensions_eq_filter_notMem
    {S : Finset (GridPoint K)} {rho A B : K}
    (hcard : S.card = 3) (hS : GridCap (K := K) S)
...
private theorem card_onConicLegalExtensions
    {S : Finset (GridPoint K)} {rho A B : K}
    (hcard : S.card = 3) (hS : GridCap (K := K) S)
    (hB : B ≠ 0) (hfit : HyperbolaFits (K := K) S rho A B) :
...
private theorem odd_card_onConicLegalExtensions
...
private theorem onConicEscapeExtensions_nonempty_of_even_bad
    (hq : Odd (Fintype.card K))
    {S : Finset (GridPoint K)} {rho A B : K}
    (hcard : S.card = 3) (hS : GridCap (K := K) S)
    (hB : B ≠ 0) (hfit : HyperbolaFits (K := K) S rho A B)
...
omit [Fintype K] [DecidableEq K] in
private theorem hyperbola_center_secant_collinear {rho A B t : K} (ht : t ≠ 0) :
    Collinear (K := K) (rho, A)
      (hyperbolaParamPoint rho A B t)
...
/-- When `2 ≠ 0`, the full nondegenerate hyperbola is maximal among residual
grid caps. -/
...
private theorem oddHyperbolaMaximalStatement :
...
omit [Fintype K] [DecidableEq K] in
private theorem psi_involutive {rho A B u : K} (hB : B ≠ 0) (hu : u ≠ 0) :
...
omit [Fintype K] [DecidableEq K] in
private theorem psi_onHyperbola_iff {rho A B u : K} (hB : B ≠ 0) (hu : u ≠ 0)
    (p : GridPoint K) :
...
omit [Fintype K] [DecidableEq K] in
private theorem psi_hyperbolaParamPoint {rho A B u t : K}
...
omit [Fintype K] [DecidableEq K] in
private theorem psi_hyperbolaParamPoint_fixed_iff_param_mul {rho A B u t : K}
    (hB : B ≠ 0) (hu : u ≠ 0) (ht : t ≠ 0) :
    psi (K := K) rho A B u (hyperbolaParamPoint rho A B t) =
        hyperbolaParamPoint rho A B t ↔
      u = t * t := by
...
omit [Fintype K] [DecidableEq K] in
private theorem eq_param_mul_of_psi_fixed_onHyperbola {rho A B u : K}
...
omit [Fintype K] [DecidableEq K] in
private theorem psi_fixed_onHyperbola_iff_param_mul {rho A B u : K}
    (hB : B ≠ 0) (hu : u ≠ 0) {p : GridPoint K}
    (hp : OnHyperbola (K := K) rho A B p) :
...
omit [Fintype K] [DecidableEq K] in
private theorem psi_ne_self_of_onHyperbola_of_param_mul_ne {rho A B u : K}
    (hB : B ≠ 0) (hu : u ≠ 0) {p : GridPoint K}
    (hp : OnHyperbola (K := K) rho A B p)
...
omit [Fintype K] [DecidableEq K] in
private theorem psi_bijective {rho A B u : K} (hB : B ≠ 0) (hu : u ≠ 0) :
    Function.Bijective (psi (K := K) rho A B u) :=
  (psi_involutive (K := K) hB hu).bijective

omit [Fintype K] [DecidableEq K] in
private theorem psi_first_eq_iff {rho A B u : K} (hB : B ≠ 0) (hu : u ≠ 0)
    {p q : GridPoint K} :
...
omit [Fintype K] [DecidableEq K] in
private theorem psi_second_eq_iff {rho A B u : K} (hB : B ≠ 0) (hu : u ≠ 0)
    {p q : GridPoint K} :
...
omit [Fintype K] [DecidableEq K] in
private theorem collinear_psi_iff {rho A B u : K} (hB : B ≠ 0) (hu : u ≠ 0)
    (p q r : GridPoint K) :
...
omit [Fintype K] in
private theorem gridSymmetry_comp {f g : GridPoint K -> GridPoint K}
...
private def coordSwap (p : GridPoint K) : GridPoint K :=
  (p.2, p.1)

omit [Field K] [Fintype K] [DecidableEq K] in
private theorem coordSwap_bijective :
...
omit [Field K] [Fintype K] [DecidableEq K] in
private theorem coordSwap_involutive (p : GridPoint K) :
    coordSwap (K := K) (coordSwap (K := K) p) = p := by
  ext <;> rfl

omit [Fintype K] [DecidableEq K] in
private theorem collinear_coordSwap_iff (p q r : GridPoint K) :
...
omit [Fintype K] in
private theorem rowSparse_image_coordSwap {S : Finset (GridPoint K)}
    (hS : GridCap (K := K) S) :
...
omit [Fintype K] in
private theorem colSparse_image_coordSwap {S : Finset (GridPoint K)}
    (hS : GridCap (K := K) S) :
...
omit [Fintype K] in
private theorem affineCap_image_coordSwap {S : Finset (GridPoint K)}
    (hS : GridCap (K := K) S) :
...
omit [Fintype K] in
private theorem gridCap_image_coordSwap {S : Finset (GridPoint K)}
    (hS : GridCap (K := K) S) :
...
omit [Fintype K] in
private theorem gridCap_image_coordSwap_iff (S : Finset (GridPoint K)) :
    GridCap (K := K) (S.image (coordSwap (K := K))) ↔
      GridCap (K := K) S := by
  constructor
  · intro hSimage
...
omit [Fintype K] in
private theorem coordSwap_gridSymmetry :
...
omit [Fintype K] [DecidableEq K] in
private theorem axisAffine_bijective {rowScale rowShift colScale colShift : K}
...
omit [Fintype K] [DecidableEq K] in
private theorem axisAffine_first_eq_iff {rowScale rowShift colScale colShift : K}
...
omit [Fintype K] [DecidableEq K] in
private theorem axisAffine_second_eq_iff {rowScale rowShift colScale colShift : K}
...
omit [Fintype K] [DecidableEq K] in
private theorem collinear_axisAffine_iff {rowScale rowShift colScale colShift : K}
...
omit [Fintype K] in
private theorem rowSparse_image_axisAffine {rowScale rowShift colScale colShift : K}
...
omit [Fintype K] in
private theorem colSparse_image_axisAffine {rowScale rowShift colScale colShift : K}
...
omit [Fintype K] in
private theorem affineCap_image_axisAffine {rowScale rowShift colScale colShift : K}
...
omit [Fintype K] in
private theorem gridCap_image_axisAffine {rowScale rowShift colScale colShift : K}
    (hrow : rowScale ≠ 0) (hcol : colScale ≠ 0) {S : Finset (GridPoint K)}
    (hS : GridCap (K := K) S) :
    GridCap (K := K)
...
omit [Fintype K] in
private theorem gridCap_image_axisAffine_iff {rowScale rowShift colScale colShift : K}
    (hrow : rowScale ≠ 0) (hcol : colScale ≠ 0)
    (S : Finset (GridPoint K)) :
    GridCap (K := K)
        (S.image (axisAffine (K := K) rowScale rowShift colScale colShift)) ↔
...
omit [Fintype K] in
private theorem axisAffine_gridSymmetry {rowScale rowShift colScale colShift : K}
    (hrow : rowScale ≠ 0) (hcol : colScale ≠ 0) :
    GridSymmetry (K := K)
...
omit [Fintype K] in
private theorem anchorAxisAffine_gridSymmetry {p q : GridPoint K}
...
omit [Fintype K] [DecidableEq K] in
private theorem anchorAxisAffine_left (p q : GridPoint K) :
    anchorAxisAffine (K := K) p q p = (0, 0) := by
  ext <;> simp [anchorAxisAffine, axisAffine]

omit [Fintype K] [DecidableEq K] in
private theorem anchorAxisAffine_right {p q : GridPoint K}
    (hrow : q.1 - p.1 ≠ 0) (hcol : q.2 - p.2 ≠ 0) :
...
omit [Fintype K] [DecidableEq K] in
private theorem gridCap_row_ne_of_ne {S : Finset (GridPoint K)} (hS : GridCap (K := K) S)
...
omit [Fintype K] [DecidableEq K] in
private theorem gridCap_col_ne_of_ne {S : Finset (GridPoint K)} (hS : GridCap (K := K) S)
    {p q : GridPoint K} (hp : p ∈ S) (hq : q ∈ S) (hpq : p ≠ q) :
    q.2 - p.2 ≠ 0 := by
  intro hzero
  apply hpq
...
omit [Fintype K] in
private theorem rowSparse_image_psi {rho A B u : K} (hB : B ≠ 0) (hu : u ≠ 0)
...
omit [Fintype K] in
private theorem colSparse_image_psi {rho A B u : K} (hB : B ≠ 0) (hu : u ≠ 0)
    {S : Finset (GridPoint K)} (hS : GridCap (K := K) S) :
...
omit [Fintype K] in
private theorem affineCap_image_psi {rho A B u : K} (hB : B ≠ 0) (hu : u ≠ 0)
    {S : Finset (GridPoint K)} (hS : GridCap (K := K) S) :
...
omit [Fintype K] in
private theorem gridCap_image_psi {rho A B u : K} (hB : B ≠ 0) (hu : u ≠ 0)
    {S : Finset (GridPoint K)} (hS : GridCap (K := K) S) :
...
omit [Fintype K] in
private theorem gridCap_image_psi_iff {rho A B u : K} (hB : B ≠ 0) (hu : u ≠ 0)
    (S : Finset (GridPoint K)) :
    GridCap (K := K) (S.image (psi (K := K) rho A B u)) ↔
      GridCap (K := K) S := by
  constructor
...
omit [Fintype K] in
private theorem psi_gridSymmetry {rho A B u : K} (hB : B ≠ 0) (hu : u ≠ 0) :
    GridSymmetry (K := K) (psi (K := K) rho A B u) :=
  ⟨psi_bijective (K := K) hB hu, gridCap_image_psi_iff (K := K) hB hu⟩

private theorem psi_mem_onConicBadExtensions_of_seed_image
...
private theorem even_onConicBadExtensions_of_psi_pairing
    {S : Finset (GridPoint K)} {rho A B u : K}
    (hB : B ≠ 0) (hu : u ≠ 0)
    (hSimage : S.image (psi (K := K) rho A B u) = S)
    (hfpf : ∀ p : GridPoint K,
...
private theorem even_onConicBadExtensions_of_psi_seed_image_no_fixed_bad_param
    {S : Finset (GridPoint K)} {rho A B u : K}
    (hB : B ≠ 0) (hu : u ≠ 0)
    (hSimage : S.image (psi (K := K) rho A B u) = S)
...
/-- For every fitted three-point conic seed, there is a nonzero parameter
`u` such that `psi rho A B u` preserves the seed, preserves its N-valued
on-conic children, and has no fixed point on those children. -/
...
omit [Fintype K] in
private theorem psiInvolutionStatement : PsiInvolutionStatement (K := K) := by
...
/-- Every three-point residual grid cap lies on a fitted nondegenerate
hyperbola with a legal on-conic child that is a P-position. -/
...
/-- Over a field of odd cardinality, evenness of the N-valued on-conic
children for every fitted three-point seed implies the on-conic escape
statement. -/
...
/-- Over a field of odd cardinality, the `psi` pairing criterion implies the
on-conic escape statement. -/
