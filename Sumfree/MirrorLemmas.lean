...
/-!
# Mirror lemmas for finite sum-free placement games

The module proves local copycat criteria for negation, translation, and affine
involutions over `F₃`. Each criterion identifies the fixed locus that must
already be unavailable and proves that the paired response preserves the
sum-free condition.
-/
...
/-- In an additive group of exponent three, doubling an element equals its
negation. -/
...
/-- The affine reflection `y ↦ -o-y` is an involution. -/
theorem sigmaF3_involutive (o y : V) :
    sigmaF3 o (sigmaF3 o y) = y := by
  unfold sigmaF3
  abel

/-- Adjoining a point together with its affine-reflection partner preserves
