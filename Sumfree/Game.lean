...
omit [Fintype G] in
/-- A finite-game move is equivalent to legality relative to the underlying
set of selected group elements. -/
theorem move_iff_legal {S : Finset G} {x : G} :
    Move S x ↔ Legal (S : Set G) x := by
  simp [Move, FiniteBuildGame.Move, Valid, Legal]

omit [Fintype G] in
private theorem legal_of_move {S : Finset G} {x : G} (h : Move S x) :
    Legal (S : Set G) x :=
  move_iff_legal.1 h

omit [Fintype G] in
private theorem move_of_legal {S : Finset G} {x : G} (h : Legal (S : Set G) x) :
    Move S x :=
  move_iff_legal.2 h

omit [Fintype G] [DecidableEq G] in
private theorem valid_empty : Valid (∅ : Finset G) := by
  intro a b c ha
  simp at ha

omit [Fintype G] [DecidableEq G] in
private theorem valid_singleton_of_ne_zero {o : G} (ho0 : o ≠ 0) :
    Valid ({o} : Finset G) := by
...
omit [Fintype G] [DecidableEq G] in
private theorem valid_singleton_iff {o : G} :
...
omit [Fintype G] in
private theorem empty_move_iff_ne_zero {o : G} :
    Move (∅ : Finset G) o ↔ o ≠ 0 := by
  simp [Move, FiniteBuildGame.Move, valid_singleton_iff]

omit [Fintype G] [DecidableEq G] in
/-- The identity element is never a legal move in a sum-free placement game. -/
...
omit [Fintype G] [DecidableEq G] in
private theorem negInvariant_empty :
    NegInvariant ((∅ : Finset G) : Set G) := by
  intro x hx
  simp at hx

omit [Fintype G] [DecidableEq G] in
private theorem negInvariant_insert_pair {A : Set G} {y : G}
    (hA : NegInvariant A) :
...
omit [Fintype G] in
private theorem negGood_step_of_live_no_obstructions
...
private theorem negGood_isP_of_live_no_obstructions
    (hfixed : ∀ {S : Finset G} {y : G}, NegGood S -> Move S y -> -y ≠ y)
    (hnoO3 : ∀ {S : Finset G} {y : G}, NegGood S -> Move S y -> y + y ≠ -y)
...
private def NegGoodWith (m : G) (S : Finset G) : Prop :=
  NegGood S ∧ m ∈ S

omit [Fintype G] [DecidableEq G] in
private theorem negInvariant_orderTwo_singleton {m : G} (hm2 : m + m = 0) :
...
omit [Fintype G] in
private theorem negGoodWith_step_of_no_other_obstructions {m : G}
...
private theorem orderTwo_singleton_isP_of_negation_no_other_obstructions {m : G}
    (hm2 : m + m = 0) (hm0 : m ≠ 0)
    (hfixed : ∀ x : G, x ≠ 0 -> x ≠ m -> -x ≠ x)
    (hnoO3 : ∀ x : G, x ≠ 0 -> x ≠ m -> x + x ≠ -x) :
    IsP ({m} : Finset G) := by
...
private theorem initial_win_of_orderTwo_no_other_negation_obstructions {m : G}
    (hm2 : m + m = 0) (hm0 : m ≠ 0)
    (hfixed : ∀ x : G, x ≠ 0 -> x ≠ m -> -x ≠ x)
    (hnoO3 : ∀ x : G, x ≠ 0 -> x ≠ m -> x + x ≠ -x) :
    Win (∅ : Finset G) := by
...
omit [Fintype G] [DecidableEq G] in
private theorem anchoredNegInvariant_insert_pair {A : Set G} {t y : G}
    (hA : ∀ {z : G}, z ∈ A -> z ≠ t -> -z ∈ A) :
...
omit [Fintype G] in
private theorem anchoredNegGood_step_of_live_obstructions {t : G}
    (ht2 : t + t = -t)
    (hfixed :
...
private theorem anchoredNegGood_isP_of_live_obstructions {t : G}
    (ht2 : t + t = -t)
    (hfixed :
      ∀ {S : Finset G} {y : G}, AnchoredNegGood t S -> Move S y -> -y ≠ y)
    (hnoO3 :
...
omit [Fintype G] in
private theorem anchoredNegGoodWith_step_of_live_obstructions {t m : G}
    (ht2 : t + t = -t)
    (hfixed :
...
private theorem anchoredNegGoodWith_isP_of_live_obstructions {t m : G}
    (ht2 : t + t = -t)
    (hfixed :
      ∀ {S : Finset G} {y : G}, AnchoredNegGoodWith t m S -> Move S y -> -y ≠ y)
    (hnoO3 :
...
private theorem orderThree_singleton_isP_of_live_obstructions {t : G}
    (ht2 : t + t = -t) (ht0 : t ≠ 0)
    (hfixed :
      ∀ {S : Finset G} {y : G}, AnchoredNegGood t S -> Move S y -> -y ≠ y)
    (hnoO3 :
...
private theorem initial_win_of_orderThree_anchor {t : G}
    (ht2 : t + t = -t) (ht0 : t ≠ 0)
    (hfixed :
      ∀ {S : Finset G} {y : G}, AnchoredNegGood t S -> Move S y -> -y ≠ y)
    (hnoO3 :
...
private theorem mem_nonzeroOrderTwoElements {v : G} :
...
private theorem mem_nonzeroOrderThreeElements {v : G} :
...
omit [Fintype G] [DecidableEq G] in
private theorem tauInvariant_empty (v : G) :
    TauInvariant v (∅ : Set G) := by
  intro x
  simp

omit [Fintype G] [DecidableEq G] in
private theorem tauInvariant_insert_pair {A : Set G} {v y : G}
...
omit [Fintype G] in
private theorem tauGood_step {v : G} (hv2 : v + v = 0) (hv0 : v ≠ 0) :
...
private theorem tauGood_isP {v : G} (hv2 : v + v = 0) (hv0 : v ≠ 0)
...
omit [Fintype G] in
private theorem tauPairGood_from_empty {v x : G}
    (hv2 : v + v = 0) (hv0 : v ≠ 0)
...
private theorem win_after_nonexception_opening_of_orderTwoMirror {v x : G}
    (hv2 : v + v = 0) (hv0 : v ≠ 0)
    (hxmove : Move (∅ : Finset G) x) (hx_ne_v : x ≠ v) :
...
private theorem neg_mem_nonzeroOrderThreeElements {t : G}
...
private theorem nonzeroOrderThree_ne_neg {t : G}
    (ht : t ∈ NonzeroOrderThreeElements (G := G)) :
    t ≠ -t := by
  rcases (mem_nonzeroOrderThreeElements (G := G) (v := t)).1 ht with ⟨ht2, ht0⟩
  intro htneg
