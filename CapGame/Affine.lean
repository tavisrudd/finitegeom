...
A position is a finite subset of an affine space containing no three distinct
collinear points. A move adds one point while preserving this cap condition,
and the player making the last legal move wins. The module specializes
`FiniteBuildGame` to this validity predicate and proves the affine
translation-mirror outcome theorem.
-/
...
/-- Every subset of an affine cap is an affine cap. -/
...
/-- Every set of at most two affine points is a cap. -/
...
@[simp] private theorem cap_empty [DecidableEq V] : Cap K V (∅ : Finset V) :=
  cap_of_card_le_two (K := K) (V := V) (by simp)

@[simp] private theorem cap_singleton [DecidableEq V] (a : V) : Cap K V ({a} : Finset V) :=
  cap_of_card_le_two (K := K) (V := V) (by simp)

/-- Any pair of affine points is a cap. -/
theorem cap_pair [DecidableEq V] (a b : V) : Cap K V ({a, b} : Finset V) :=
  cap_of_card_le_two (K := K) (V := V) (by
    by_cases h : a = b <;> simp [h])

/-- Affine collinearity is invariant under exchanging its first two points. -/
...
/-- Affine collinearity is invariant under cyclically rotating the three points. -/
...
/-- A fresh point may be adjoined to a cap when it forms no collinear triple
with two distinct old points. -/
...
private theorem collinear_affineEquiv (e : V ≃ᵃ[K] V) {a b c : V} :
...
private theorem cap_image_affineEquiv [DecidableEq V] (e : V ≃ᵃ[K] V) {S : Finset V}
...
private theorem mem_of_image_eq_self [DecidableEq V] (e : V ≃ᵃ[K] V) {S : Finset V}
    (hinv : S.map e.toEquiv.toEmbedding = S) {x : V} (hx : x ∈ S) :
    e x ∈ S := by
  rw [← hinv]
  exact Finset.mem_map_of_mem _ hx

private theorem mem_iff_of_image_eq_self [DecidableEq V] (e : V ≃ᵃ[K] V) {S : Finset V}
...
private theorem no_old_point_on_mirror_line [DecidableEq V] (e : V ≃ᵃ[K] V)
...
private theorem collinear_midpoint_left [Invertible (2 : K)] (x y : V) :
...
private theorem mirror_move_legal [Fintype V] [DecidableEq V] (e : V ≃ᵃ[K] V)
...
/-- Membership in the affine legal-extension set is equivalent to freshness
and preservation of the cap condition. -/
...
private theorem mirrorGood_step (e : V ≃ᵃ[K] V) (hinvol : ∀ z : V, e (e z) = z) :
...
private theorem isP_of_mirrorGood (e : V ≃ᵃ[K] V) (hinvol : ∀ z : V, e (e z) = z)
