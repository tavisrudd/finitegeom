...
/-!
# Axiom audit for the structural Clebsch passages

This audit prints the axioms of the terminals exported with the structural
Clebsch passages boundary: the quadratic and split pinching results, the
involutive odd-unit splitting, the golden quadratic characters and reflection
factorization, the tight-frame conference identity, the signed equiangular
Gram determinants, the conference square and switching invariance of the
triangle cubic, the marked chart comparisons, the Kneser pair eigenspace and
Petersen harmonic scalars, the marked invariant cubic and its Gaunt
coefficient, the aligned two-graph classification and query counts, and the
chart correspondence transports.
-/
