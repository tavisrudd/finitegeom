...
/-!
# Completion distance as a transversal number: `δ_x = τ(𝓗_C(x))`

This module identifies the deletion cost of inserting an external symbol into
an independent facet with the transversal number of its circuit-trace
hypergraph.

## Abstract model

For a facet `C` and an external symbol `x ∉ C`, the **circuit-trace hypergraph** is
`𝓗_C(x) = {A ⊆ C : A ∪ {x} is a circuit}`.  The identity proved here is
...
survives the deletion (`A ⊄ C ∖ D`).  Thus
`completionDistance = transversalNumber` is a finite hypergraph statement with no imported
matroid input.  A concrete instance supplies the equivalence between independence and the absence
of a surviving trace; this module treats `𝓗_C(x)` as any `Finset`-hypergraph with edges inside `C`.
-/
...
/-- **Completion distance equals transversal number.** With every circuit trace inside `C` and
nonempty, the completion distance equals the transversal number of the circuit-trace hypergraph.
The `≥ τ` direction is immediate (a witnessing deletion *is* a
