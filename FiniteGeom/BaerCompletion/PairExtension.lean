...
/-- Removing a set of cardinality at most `M` from a set of cardinality `N` leaves at least
`N - M` elements. -/
...
`QuadraticBaerPairExtensionData` are established, the displayed lower bound follows without further
...
/-- A positive empty-line count and a strict orbit-count bound imply existence of a legal
conjugate pair. -/
