...
This module connects `FiniteGeom.Completion` to finite hereditary independence systems:
inserting `x` after deleting `D` is independent exactly when no dependent trace survives.  We
work with all dependent traces, rather than only inclusion-minimal circuits; this gives the same
transversals without requiring a separate matroid circuit API.
-/
...
omit [Fintype V] in
/-- Membership in the obstruction hypergraph is dependence of the inserted trace together with
containment in the original set. -/
...
omit [Fintype V] in
/-- Every obstruction trace lies in the original set. -/
theorem obstruction_edge_subset {C A : Finset V} {x : V}
    (hA : A ∈ obstructionHypergraph I C x) : A ⊆ C :=
  (mem_obstructionHypergraph I).mp hA |>.1

omit [Fintype V] in
/-- If the inserted singleton is independent, every obstruction trace is nonempty. -/
...
omit [Fintype V] in
/-- Insertion is independent exactly when no dependent trace survives the deletion. -/
