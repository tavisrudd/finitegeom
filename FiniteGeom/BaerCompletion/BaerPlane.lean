...
/-- A point fixed by the incidence involution. -/
def IsFixedPoint (p : P) : Prop := G.pointConj p = p

/-- A line fixed by the incidence involution. -/
...
omit [Fintype P] [DecidableEq P] [DecidableEq L] [DecidableRel G.incident] in
/-- In an invariant finite set, a point belongs exactly when its conjugate belongs. -/
