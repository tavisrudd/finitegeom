...
After the two squared-angle values for the ten labelled face axes are
supplied, evaluating the normalized degree-six Legendre polynomial gives
`-65/243` and `47/243`.  Pairs of two-subsets are either disjoint or meet in
one point, so the resulting algebraic kernel operator is
...
The proofs below establish this algebraic consequence from the two orbit
values and the symbolic Kneser eigenspace theorems; no ten-by-ten
diagonalization is performed.  They do not identify geometric face axes or
prove the spherical addition theorem that supplies the kernel interpretation.
-/
